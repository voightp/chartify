...
def populated_mw(mw, excel_file, eso_file1, totals_file, qtbot):
    # mw.load_files_from_paths([excel_file,eso_file1]
...
def test_empty_current_model(mw):
    assert not mw.current_model


def test_mirror_layout(mw):
...
def test_get_all_models(all_files, all_tables, n_models, populated_mw):
    populated_mw.toolbar.all_files_toggle.setChecked(all_files)
    populated_mw.toolbar.all_tables_toggle.setChecked(all_tables)
    assert n_models == len(populated_mw.get_all_models())


def test_add_new_tab(mw, eso_file: EsoFile):
...
def test_on_tree_act_checked(qtbot, mw, checked: bool, node: Optional[str], enabled: bool):
...
def test_on_selection_populated(
    qtbot, mw, variables: List[VariableData], enabled: bool, rate_to_energy: bool
):
    def cb(vd):
        return vd == variables
...
def test_on_totals_checked(qtbot, mw):
    pytest.fail()


def test_on_all_files_checked(mw):
    pytest.fail()
...
def test_confirm_remove_variables(mw, confirmed: int, expected: bool):
...
def test_confirm_rename_variable_simple_variable(
    mw, confirmed: int, expected: Optional[Tuple[str, None]]
):
    with patch("chartify.ui.main_window.SingleInputDialog") as dialog:
        instance = dialog.return_value
        instance.exec_.return_value = confirmed
...
def test_confirm_rename_variable(mw, confirmed: int, expected: Optional[Tuple[str, None]]):
